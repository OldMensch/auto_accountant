...
class AssetEditor(tk.Toplevel):
    ...
    def updateProfileInstance(self, newID=None):
        ...
        for p in profiles:
            if profiles[p]["assets"].count(self.oldID) == 1:    #if this asset is in there, pop it
    ...
    def comm_deleteAsset(self):
        ...
        TEMP["widgets"][self.oldID]["transFrame"].destroy() #Must destroy tkinter widgets to prevent memory leaks!
        TEMP["widgets"][self.oldID]["assetFrame"].destroy()
        TEMP["widgets"].pop(self.oldID)                     #removal from temporary data
        TEMP["metrics"].pop(self.oldID)
    ...
    def comm_save(self):
        ...
        if self.oldID != 1 and self.oldID.split("z")[1] != ID.split("z")[1]:
            for t in PERM["assets"][self.oldID]["trans"]:
                if assetclasslib[ID.split("z")[1]]["validTrans"].count(PERM["assets"][self.oldID]["trans"][t]["type"]) == 0:    #if transaction t's trans. type is NOT one of the valid types...
        ...
        #==============================
        #check: has the ID been changed, or is the file new?? if yes: delete old transaction, insert new one with this data. if no, just update the information
        if self.oldID == 1 or self.oldID != ID:     #NEW: The file is a new file
            PERM["assets"][ID] = TEMP2
            self.upper.create_ASSET_WIDGETS(ID)
            if self.oldID != 1: #ID CHANGE: The ID was modified. adds in the new transaction with old trans data, then deletes old data
                PERM["assets"][ID]["trans"] = PERM["assets"][self.oldID]["trans"]   #copy the old transaction data over directly
                for t in PERM["assets"][self.oldID]["trans"]:   #have to create new widgets for migrated transactions, the old one's partial commands wont work
                    self.upper.create_TRANS_WIDGET(ID, t)
                PERM["assets"].pop(self.oldID)             #removal of old data
                TEMP["widgets"][self.oldID]["transFrame"].destroy() #removal of old widgets. .destroy is NECESSARY to prevent memory leaks!
                TEMP["widgets"][self.oldID]["assetFrame"].destroy()
                TEMP["widgets"].pop(self.oldID)           #removal of the rest of the old widget tree
            ...
            TEMP2.pop("trans")
            PERM["assets"][self.oldID].update(TEMP2)
            self.upper.reconfig_ASSET(self.oldID)
