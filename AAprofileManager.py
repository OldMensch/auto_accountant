...
class ProfileManager(tk.Toplevel):
    ...
    def setup(self):    #Colors any selected wallets white
        for w in PERM['profiles'][self.profile]['wallets']: self.WALLETS[w].configure(bg='#ffffff', fg='#000000')
        for a in PERM['profiles'][self.profile]['assets']:  self.ASSETS[a].configure( bg='#ffffff', fg='#000000')
        for c in PERM['profiles'][self.profile]['classes']: self.CLASSES[c].configure(bg='#ffffff', fg='#000000')
...
#=============================================================
    ...
    def comm_close(self):  
        ...
        if self.upper.profile not in PERM['profiles']:      self.upper.profile = ''
